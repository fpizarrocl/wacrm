// Resolve the base URL of the running app from an inbound request.
//
...
//      generated links Just Work in production without forcing
//      the operator to set an env var.
...
//   receive a redirect/link pointing at their site.
//
...
//
// Shared by the invite-link generator and the OAuth callback route
// — both need to turn "whatever host this request actually arrived
// on" into an absolute URL, and both sit behind the same kind of
// reverse-proxy setups where `new URL(request.url).origin` reflects
// the app's internal bind address (e.g. 0.0.0.0:3000) rather than
// the public domain.
...
export function getBaseUrl(request: Request): string {
  ...
  // probing the app with a spoofed Host header.
