...
export type ContactChannel = 'whatsapp' | 'instagram' | 'messenger';

export interface Contact {
  id: string;
  user_id: string;
  account_id: string;
  /** Only set for `channel === 'whatsapp'` (migration 046 made it
   *  nullable — Instagram/Messenger contacts have no phone). */
  phone?: string;
  /** Digits-only form of `phone`, generated by the DB (migration 022)
   *  and unique per account. Read-only. */
  phone_normalized?: string;
  /** Defaults to 'whatsapp' for every pre-046 row. */
  channel?: ContactChannel;
  /** IGSID/PSID Meta assigns per user per channel — only set for
   *  `channel !== 'whatsapp'` (migration 046). */
...
export interface Conversation {
  ...
  /** Denormalized from the contact (migration 046). Defaults to
   *  'whatsapp' for every pre-046 row. */
  channel?: ContactChannel;
